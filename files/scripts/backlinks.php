...
$topLevelName = $current_domain_plain;		// name of home/root directory
...
if (isset($_GET['tid']))
{
	$backlinks_sql = "SELECT post_title FROM cms_posts WHERE post_topic_id = '$threadid' AND post_is_thread = '1' LIMIT 1";
	$backlinks_result = mysql_query($backlinks_sql);
	$backlinks_result = mysql_result($backlinks_result, 0);
	$thread_linky = ' &gt; <a href="' . $current_domain . 'forum/topic/' . ($_GET['tid']) . '">' . $backlinks_result . '</a>';
}
else
{
	$thread_linky = '';
...
if ($results != '') print('<div class="backlinks">You are at: ' . $results . $thread_linky . '</div>');
